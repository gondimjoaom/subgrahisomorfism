...
graphviz = sys.argv[5] # number of edges in smaller graph
...
print_digraph = graphviz # if True, print in graphviz format for visualization
...
def main():
  ...
  if print_digraph:
    ...
    finalgraphfile = ''
    for e in G1:
      finalgraphfile += str(e[0]) + ' ' + str(e[1]) + '\n'
    finalgraphfile += '0 0' + '\n'
    for e in G2:
      finalgraphfile += str(e[0]) + ' ' + str(e[1]) + '\n'
    with open(f'{graphviz}', 'w') as graphFile:
